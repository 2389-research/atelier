...
description: >
  ...
  fast / lean / dispatch", or a multi-unit build where cost matters. The architect
  (Opus) writes a contract + a units.jsonl and calls a dispatch SCRIPT that loops
  cheap bare model calls (claude -p --bare, subscription, no API key) to write each
  unit to disk — NO subagents. The orchestrator never ingests unit outputs, only a
...
writes a spec; a deterministic script runs the per-unit model calls.** It exists
...
3-unit task). This architecture avoids both: bare calls (~$0.02–0.05 each) and
outputs go to disk, not into your context.

## Why it's cheap (the two fixes)
1. **Bare calls.** Each unit runs `claude -p "<prompt>" --model {haiku|sonnet} --bare`
   — `--bare` strips hooks/plugins/system-prompt, so a Haiku unit call is ~$0.002–0.05
...
2. **Write `contract.md`** — the cross-unit surface ONLY (shared interfaces/seam,
...
3. **Write `units.jsonl`** — one JSON object per line:
   ```json
   ...
   - `deps`: unit ids that must finish first (script runs in dependency waves, parallel
     within a wave).
   - For a split-planning pass, first emit `kind:"brief"` units (tier sonnet) to write
     briefs, then `kind:"generate"` units (tier haiku) that consume them. For small
...
4. **Dispatch** — run the script ONCE via Bash:
   ...
   ```
   It loops the units, makes the bare calls (parallel within a dep wave), writes files
...
6. **Surgical-fix loop** — for each failure, read ONLY the failing test + the relevant
   ...
   re-dispatch that one unit with a corrective note appended to its brief. Re-run the
...
## Verified (task 01, wordfreq, 3 units)
...
- **Executors emit text, not tool use.** `--bare` strips tools; the model outputs
  `<FILE>` blocks and the script writes them. Don't expect the unit call to run tests
